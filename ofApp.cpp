...
void ofApp::update() {
	...
	for (int radius = 50; radius < 200; radius += 40) {

		this->setRingToMesh(this->face, this->frame, glm::vec3(), radius, 18, 20);
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height) {
	...
	for (int deg = 0; deg < 360; deg += 1) {
		...
		for (auto& vertex : vertices) {
			...
			auto angle_x = ofMap(ofNoise(noise_seed.x + vertex.x * ofMap(radius, 50, 350, 0.0005, 0.00035), ofGetFrameNum() * ofMap(radius, 50, 350, 0.0035, 0.001)), 0, 1, -PI, PI);
			auto angle_y = ofMap(ofNoise(noise_seed.y + vertex.y * ofMap(radius, 50, 350, 0.0005, 0.00035), ofGetFrameNum() * ofMap(radius, 50, 350, 0.0035, 0.001)), 0, 1, -PI, PI);
			auto angle_z = ofMap(ofNoise(noise_seed.z + vertex.z * ofMap(radius, 50, 350, 0.0005, 0.00035), ofGetFrameNum() * ofMap(radius, 50, 350, 0.0035, 0.001)), 0, 1, -PI, PI);
